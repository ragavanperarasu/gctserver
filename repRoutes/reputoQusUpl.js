...
router.post('/reputoqusupl', async (req, res, next) => {
    ...
    if(pdept === "Civil Engineering"){
        sub = await civilModel.updateOne({sem:psem, "sub.subname":psub},{$push:{"sub.$.utoqus":js}})
    }
    else if(pdept === "Mechanical Engineering"){
        sub = await mechModel.updateOne({sem:psem, "sub.subname":psub},{$push:{"sub.$.utoqus":js}})
       
    }
    else if(pdept === "Electrical And Electronics Engineering"){
        sub = await eeeModel.updateOne({sem:psem, "sub.subname":psub},{$push:{"sub.$.utoqus":js}})
        
    }
    else if(pdept === "Electronics And Communication Engineering"){
        sub = await eceModel.updateOne({sem:psem, "sub.subname":psub},{$push:{"sub.$.utoqus":js}})
    }
    else if(pdept === "Production Engineering"){
        sub = await prodModel.updateOne({sem:psem, "sub.subname":psub},{$push:{"sub.$.utoqus":js}})
    }
    else if(pdept === "Electronics And Instrumentation Engineering"){
        sub = await eandiModel.updateOne({sem:psem, "sub.subname":psub},{$push:{"sub.$.utoqus":js}})
    }
    else if(pdept === "Computer Science Engineering"){
        sub = await cseModel.updateOne({sem:psem, "sub.subname":psub},{$push:{"sub.$.utoqus":js}})
    }
    else if(pdept === "Information Technology"){
        sub = await itModel.updateOne({sem:psem, "sub.subname":psub},{$push:{"sub.$.utoqus":js}})
    }
    else if(pdept === "Industrial Bio Technology"){
        sub = await ibtModel.updateOne({sem:psem, "sub.subname":psub},{$push:{"sub.$.utoqus":js}})
    } 
